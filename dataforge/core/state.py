...
class GraphState(BaseModel):
    ...
    def add_agent_result(
        ...
        **metadata: Any,
        ...
        # Propagate the agent's decision when the caller did not pass one
        # explicitly (execute_with_logging packs it inside `result`).
        if decision == "continue" and isinstance(result.get("decision"), str):
            decision = result["decision"]
        # Keep the full result payload for routers (planner reads
        # next_agent_suggestion / success from here).
